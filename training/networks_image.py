...
class DenseLabelEmbedder(nn.Module):
    def __init__(self, label_channels, hidden_size):
        ...
        self.conv_layers = nn.ModuleList([
            MPConv(label_channels, hidden_size // 2, [3, 3]),
            MPConv(hidden_size // 2, hidden_size // 2, [3, 3]),
            MPConv(hidden_size // 2, hidden_size, [3, 3]),
        ])
        self.conv = MPConv(hidden_size, hidden_size, [1,1])

    def forward(self, labels):
        for i in range(0, len(self.conv_layers)):
            labels = self.conv_layers[i](labels)
            labels = normalize(labels, dim=1)  # pixel norm
            labels = mp_silu(labels)
        labels = self.conv(labels)
        return labels
...
class Block(torch.nn.Module):
    def __init__(self,
        ...
        self.emb_linear = MPConv(emb_channels, out_channels, kernel=[1,1])
    ...
    def forward(self, x, emb):
        ...
        # 判断空间维度 (H, W) 是否成倍数关系
        emb = F.adaptive_avg_pool2d(emb, (y.shape[2], y.shape[3]))
...
class UNet(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs,                     # Arguments for Block.
        ...
        self.emb_label = DenseLabelEmbedder(label_dim,cemb) if label_dim != 0 else None
        # Encoder.
    ...
    def forward(self, x, noise_labels, class_labels):
        ...
        if self.emb_label is not None:
            labels_emb = self.emb_label(class_labels * np.sqrt(class_labels.shape[1]))
            emb = mp_sum(emb.unsqueeze(2).unsqueeze(3), labels_emb, t=self.label_balance)
        emb = mp_silu(emb)
        # Encoder.
...
class Precond(torch.nn.Module):
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, return_logvar=False, **unet_kwargs):
        ...
        class_labels = torch.zeros((x.shape[0],1,x.shape[-2],x.shape[-1]),device=x.device) if class_labels is None else class_labels.to(torch.float32)
...
def modified_forward(self, x, emb):
    # Main branch.
    x = resample(x, f=self.resample_filter, mode=self.resample_mode)
    ...
    # but we haven't done sufficient testing to verify that it produces identical results.
    if self.num_heads != 0:
        y_qkv = self.attn_qkv(x)  # shape: [B, C, H, W]
        B, C, H, W = y_qkv.shape
        N = H * W
        y_qkv = y_qkv.reshape(B, self.num_heads, -1, 3, N)
        q, k, v = normalize(y_qkv, dim=2).unbind(3)  # [B, num_heads, d, N]
        assert B % 2 == 0, "Batch size must be even."
        B_half = B // 2
        k = torch.cat([k, k[B_half:].repeat(2, 1, 1, 1)], dim=-1)
        v = torch.cat([v, v[B_half:].repeat(2, 1, 1, 1)], dim=-1)
...
def register_attention(unet_model, modified_forward=modified_forward):
