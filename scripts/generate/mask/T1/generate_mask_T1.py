        ...
        def __iter__(self):
            ...
            for batch_idx, indices in enumerate(rank_batches):
                ...
                if len(r.seeds) > 0:
                    ...
                    r.noise = rnd.randn([len(r.seeds),net.img_channels,net.img_resolution,net.img_resolution], device=device)#.repeat(len(r.seeds), 1, 1, 1)
                    # Generate images.
                    ...
                    images = encoder_img.decode_pixels(latents).squeeze(1)
                    r.images  = torch.where(images >= num_classes, torch.zeros_like(images ), images) 
                    # Save images.
                    if outdir is not None:
                        for seed, image in zip(r.seeds, r.images.cpu().numpy()):
                            image_dir = os.path.join(outdir, 'demo') 
                            mask_dir = os.path.join(outdir, 'msk')
                            mask = PIL.Image.fromarray(image.astype(np.uint8), 'L')  # 转换为 8-bit 灰度图
                            os.makedirs(image_dir, exist_ok=True)
                            os.makedirs(mask_dir, exist_ok=True)
                            mask.save(os.path.join(mask_dir, f'{seed:06d}.tif'))
                            image = make_rgb(image)
                            PIL.Image.fromarray(image, 'RGB').save(os.path.join(image_dir, f'{seed:06d}_img.png'))
                # Yield results.
