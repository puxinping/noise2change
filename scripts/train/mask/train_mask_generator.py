...
import training.training_loop_mask
...
def setup_training_config(preset='edm2-img512-s', **opts):
    ...
    # Dataset.
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset_mask.CustomDataset', path=opts.data,  use_labels=opts.get('cond', True))
    try:
    ...
    # Encoder.
    c.encoder_kwargs = dnnlib.EasyDict(class_name='training.encoders_image.DiscreteEncoder')

    # Hyperparameters.
    c.update(total_nimg=opts.duration, batch_size=opts.batch)
    c.network_kwargs = dnnlib.EasyDict(class_name='training.networks_mask.Precond', model_channels=opts.channels, dropout=opts.dropout)
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.training_loop_mask.EDM2Loss', P_mean=opts.P_mean, P_std=opts.P_std)
    c.lr_kwargs = dnnlib.EasyDict(func_name='training.training_loop_mask.learning_rate_schedule', ref_lr=opts.lr, ref_batches=opts.decay)
...
# I/O-related options.
@click.option('--status',           help='Interval of status prints', metavar='NIMG',           type=parse_nimg, default='10Ki', show_default=True)
@click.option('--snapshot',         help='Interval of network snapshots', metavar='NIMG',       type=parse_nimg, default='100Ki', show_default=True)
@click.option('--checkpoint',       help='Interval of training checkpoints', metavar='NIMG',    type=parse_nimg, default='100Ki', show_default=True)
