...
import training.training_loop_image
...
def setup_training_config(preset='edm2-img512-s', **opts):
    ...
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset_image.CustomDataset', path=opts.data,  use_labels=opts.get('cond', True))
    try:
    ...
    # Encoder.
    c.encoder_img_kwargs = dnnlib.EasyDict(class_name='training.encoders_image.StabilityVAEEncoder')
    c.encoder_msk_kwargs = dnnlib.EasyDict(class_name='training.encoders_mask.StabilityVAEEncoder')
...
@click.option('--P_mean', 'P_mean', help='Noise level mean', metavar='FLOAT',                   type=float, default=-0.8)
@click.option('--P_std', 'P_std',   help='Noise level standard deviation', metavar='FLOAT',     type=click.FloatRange(min=0, min_open=True), default=1.6)
