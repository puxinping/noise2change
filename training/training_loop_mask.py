...
def training_loop(
    dataset_kwargs      = dict(class_name='training.dataset_mask_pixels.CustomDataset', path=None),
    encoder_kwargs      = dict(class_name='training.encoders.DiscreteEncoder'),
    data_loader_kwargs  = dict(class_name='torch.utils.data.DataLoader', pin_memory=True, num_workers=2, prefetch_factor=2),
    network_kwargs      = dict(class_name='training.networks_msk_pixels.Precond'),
    loss_kwargs         = dict(class_name='training.training_loop_mask_pixel.EDM2Loss'),
    optimizer_kwargs    = dict(class_name='torch.optim.Adam', betas=(0.9, 0.99)),
    lr_kwargs           = dict(func_name='training.training_loop_mask_pixel.learning_rate_schedule'),
    ...
    while True:
        ...
        if snapshot_nimg is not None and state.cur_nimg % snapshot_nimg == 0 and (state.cur_nimg != start_nimg or start_nimg == 0) and dist.get_rank() == 0:
            ...
            for ema_net, ema_suffix in ema_list:
                data = dnnlib.EasyDict(encoder_img=encoder_img, dataset_kwargs=dataset_kwargs, loss_fn=loss_fn)
